...
const getDailyWeather = (url) => {
    ...
    console.log(data);
            ...
            const dailyForecasts = data.list.map(fc => {
                return {
...
const getHourlyWeather = (url) => {
    return new Promise((resolve, reject) => {
        ...
        .then(response => response.json())
        .then(data => {
    console.log("RESPONSE: getHourlyWeather");
    console.log(data);
            const location = {
                name: data.city.name,
                latitude: data.city.coord.lat,
...
class WeatherService {
    ...
    getDailyWeatherByPosition({latitude, longitude}) {
        ...
        const url = `${WEATHER_BASE_URL}/forecast/daily?appid=${WEATHER_API_KEY}&lat=${latitude}&lon=${longitude}&units=imperial&cnt=7`;
